...
namespace ElsaWebApp.Services.DataAccess
{
    public class ClassroomService
    {
        ...
        {
            var users = await _http.GetFromJsonAsync<List<Classroom>>("classroom/GetAll");
            return users;
        }

        public async Task<Classroom> GetClassroomById(int id)
        {
            var user = await _http.GetFromJsonAsync<Classroom>($"classroom/{id}");
            return user;
        }

        public async Task<bool> InsertClassroom(Classroom classroom)
        {
            var responseMessage = await _http.PostAsJsonAsync("classroom", classroom);
            return responseMessage.IsSuccessStatusCode;
        }
        
        public async Task<bool> InsertClassroom(List<Classroom> classrooms)
        {
            var responseMessage = await _http.PostAsJsonAsync("classroom/addrange", classrooms);
            return responseMessage.IsSuccessStatusCode;
        }

        public async Task<bool> UpdateClassroom(Classroom classroom)
        {
            var responseMessage = await _http.PutAsJsonAsync("classroom", classroom);
            return responseMessage.IsSuccessStatusCode;
        }
    }
}